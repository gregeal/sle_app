...
void main() {
  ...
  group('mock results', () {
    test('latestMockPerSkill returns the newest result per skill', () async {
      ...
      await db.recordMockResult(
          skill: 'reading', score: 6, total: 10, levelEstimate: 'B', at: now);
      await db.recordMockResult(
          skill: 'reading',
          score: 8,
          total: 10,
      ...
      await db.recordMockResult(
          skill: 'oral', score: 0, total: 0, levelEstimate: 'B', at: now);
  ...
  group('supporting stats', () {
    ...
    test('drillStatsSince counts only attempts at or after the cutoff',
        () async {
      final db = inMemoryDatabase();
      addTearDown(db.close);
      ...
      await db.recordAttempt(itemId,
          wasCorrect: true, at: now.subtract(const Duration(days: 1)));
      await db.recordAttempt(itemId, wasCorrect: true, at: now);
      await db.recordAttempt(itemId,
          wasCorrect: false, at: now.add(const Duration(minutes: 5)));
  ...
  group('mock scoring', () {
    test('maps fractions to approximate SLE levels', () {
      ...
      expect(levelForFraction(0.55), 'B');
      expect(levelForFraction(0.4), 'A');
    });

    test('nextCheckpoint lands every 28 days after the plan start', () {
      final start = DateTime(2026, 7, 12);
      expect(nextCheckpoint(start, DateTime(2026, 7, 13)),
          DateTime(2026, 8, 9));
      expect(nextCheckpoint(start, DateTime(2026, 8, 9)),
          DateTime(2026, 8, 9));
      expect(nextCheckpoint(start, DateTime(2026, 8, 10)),
          DateTime(2026, 9, 6));
    });
  });
}
